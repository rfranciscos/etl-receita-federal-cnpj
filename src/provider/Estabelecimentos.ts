...
export class EstabelecimentosProvider {
  build = (data: string[]): Estabelecimentos => {
    return {
      ...
      tipoEmpresaId: sanitizeData(data[EEstabelecimentos.IDENTIFICADOR]) as string,
      situacaoCadastralId: sanitizeData(data[EEstabelecimentos.SITUACAO_CADASTRAL]) as string,
      dataSituacaoCadastral: sanitizeData(data[EEstabelecimentos.DATA_SITUACAO_CADASTRAL]) as string,
      motivoSituacaoCadastralId: sanitizeData(data[EEstabelecimentos.MOTIVO_SITUACAO_CADASTRAL]) as string,
      nomeCidadeExterior: sanitizeData(data[EEstabelecimentos.NOME_DA_CIDADE_NO_EXTERIOR]) as string,
      paisId: sanitizeData(data[EEstabelecimentos.PAIS]) as string,
      dataInicioAtividade: sanitizeData(data[EEstabelecimentos.DATA_DE_INICIO_ATIVIDADE]) as string,
      CnaePrincipalId: sanitizeData(data[EEstabelecimentos.CNAE_FISCAL_PRINCIPAL]) as string,
      CnaeSegundarioId: sanitizeData(data[EEstabelecimentos.CNAE_FISCAL_SECUNDARIA]) as string,
      ...
      municipioId: sanitizeData(data[EEstabelecimentos.MUNICIPIO]) as string,
